...
func (s *SmartContract) DangerUser(ctx contractapi.TransactionContextInterface, argsJSON string) error {
	var user struct {
		Username string `json:"username"`
		ID       string `json:"id"`
	}

	err := json.Unmarshal([]byte(argsJSON), &user)
	if err != nil {
	...
	// Save the user data in CouchDB
	err = ctx.GetStub().PutState(id, userBytes)
	if err != nil {
		return fmt.Errorf("failed to put user data: %v", err)
	}
